...
public class ClickTimes : MonoBehaviour {
    ...
    IEnumerator Countdown()
    {
        ...
        counter.GetComponent<GUIText>().text = (System.Int32.Parse(counter.GetComponent<GUIText>().text) - 1).ToString();  //It's more sensible to store this as an integer, but let's explore some string manipulation.
        ...
        else
        {
            ...
            GetComponent<HighScore>().Setscore(System.Int32.Parse(GetComponent<GUIText>().text)); //Plus we send our score through to our HighScore class
            toptext.GetComponent<GUIText>().text = "Enter your username."; // And we request a username
	...
	void Update () {
        if (allowedToClick && Input.GetMouseButtonUp(0)) // If the user is allowed to click and chooses to
        {
            ...
            GetComponent<GUIText>().text = (System.Int32.Parse(GetComponent<GUIText>().text) + 1).ToString(); //And the score goes up. Again, we could easily store this as an int.
        }
	}
}
